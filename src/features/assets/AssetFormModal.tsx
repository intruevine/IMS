...
export const AssetFormModal: React.FC<AssetFormModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (asset) {
        await updateAsset(asset.contractId, asset.id, formData);
        showToast('자산이 성공적으로 수정되었습니다', 'success');
      } else {
        await addAsset(selectedContractId, formData as Omit<AssetItem, 'id'>);
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-h-[75vh] overflow-y-auto pr-2">
        ...
        {!asset && (
          <div className="bg-yellow-50 rounded-xl p-6 border border-yellow-100">
            <h4 className="text-sm font-bold text-yellow-600 uppercase mb-4 flex items-center gap-2">
